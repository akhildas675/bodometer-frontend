...
import { GetWorkoutPlansResponse, WorkoutPlanResponse, WorkoutProgressResponse, MarkDayCompletedPayload, MarkExerciseStatusPayload } from "@/interface/workout.interface";
...
const userServices = {
  ...
  async getWorkoutProgress(timeframe?: string): Promise<ApiResponse<WorkoutProgressResponse>> {
    const url = timeframe ? `${USER_API_ROUTES.GET_WORKOUT_PROGRESS}?timeframe=${timeframe}` : USER_API_ROUTES.GET_WORKOUT_PROGRESS;
    const response = await userApi.get<ApiResponse<WorkoutProgressResponse>>(url);
    return response.data;
  },
};
