...
import type { TrainerWithProfile } from "@/components/ui/DataTable.types";

import RejectionModal from "@/features/admin/trainers/components/TrainerAppointmentRejectionModal";
import ConfirmationModal from "@/components/ui/ConfirmDialog";
import { VerificationStatus } from "@/constants/verification-status.constants";
...
const AdminTrainerAppointmentDetails = () => {
  ...
  const fetchTrainerDetails = async () => {
    ...
    try {
      setLoading(true);
      const response = await trainerService.getTrainerProfileById(profileId);
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const res = await trainerService.approveTrainer(trainer.profile._id);
  ...
  const handleReject = async (reason: string) => {
    ...
    try {
      setActionLoading(true);
      const res = await trainerService.rejectTrainer(trainer.profile._id, reason);
      toast.success(res.message);
      fetchTrainerDetails();
    } catch (error: unknown) {
      const apiError = parseApiError(error);
