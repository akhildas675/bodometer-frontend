...
import { exerciseColumns } from "@/features/admin/exercises/components/AdminExerciseColumns";
import { useExerciseActions, type ExerciseModalConfig } from "@/features/admin/exercises/components/AdminExerciseActions";
        ...
        <tbody>
          {[1, 2, 3, 4, 5].map((row) => (
            <tr key={row} className="border-b border-purple-500/10">
              <td className="py-4 px-6"><div className="w-12 h-12 bg-purple-500/20 rounded-lg animate-pulse"></div></td>
              <td className="py-4 px-6"><div className="h-4 bg-purple-500/20 rounded w-32 animate-pulse"></div></td>
              <td className="py-4 px-6"><div className="h-6 bg-purple-500/20 rounded-full w-24 animate-pulse"></div></td>
              <td className="py-4 px-6"><div className="h-6 bg-purple-500/20 rounded-full w-20 animate-pulse"></div></td>
              <td className="py-4 px-6"><div className="h-4 bg-purple-500/20 rounded w-48 animate-pulse"></div></td>
              <td className="py-4 px-6"><div className="h-6 bg-purple-500/20 rounded-full w-16 animate-pulse"></div></td>
            </tr>
          ))}
        </tbody>
      </table>
...
const AdminExerciseList = () => {
  ...
  const [sortConfig, setSortConfig] = useState<SortConfig<keyof ExerciseRow>>({
    field: "" as keyof ExerciseRow,
    order: "asc",
  });
  ...
  const {
    ...
    loading,
    refetch,
  } = useTableFetch<PaginatedResponse<ExerciseRow>>(fetchFn, false);

  useEffect(() => {
    refetch();
  }, [fetchFn, refetch]);

  const [modalConfig, setModalConfig] = useState<ExerciseModalConfig>({
    isOpen: false,
    title: "",
    message: "",
    onConfirm: () => {},
  ...
  const actions = useExerciseActions(refetch, setModalConfig);
  ...
  const handleSortChange = useCallback(
    (sort: SortConfig<keyof ExerciseRow>) => {
      setSortConfig(sort);
      setCurrentPage(1);
    },
    []
  ...
  const sortOptions = extractSortOptions(exerciseColumns);

  return (
    <>
      <div className="text-white">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-semibold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-400">
            Exercise Management
          </h1>
          <button
            onClick={() => navigate(ADMIN_UI_ROUTES.EXERCISES_FORM)}
        ...
        <div className="mb-6 flex gap-4">
          ...
          />
          <SortDropdown<keyof ExerciseRow>
            options={sortOptions}
            value={sortConfig}
