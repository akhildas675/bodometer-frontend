...
import { targetMuscleColumns } from "@/features/admin/target-muscles/components/AdminTargetMuscleColumns";
import { useTargetMuscleActions, type TargetMuscleModalConfig } from "@/features/admin/target-muscles/components/AdminTargetMuscleActions";
...
const AdminTargetMuscleList = () => {
  const navigate = useNavigate();

  const [searchQuery, setSearchQuery] = useState("");
  const [sortConfig, setSortConfig] = useState<SortConfig<keyof UpdateTargetMuscles>>({
    field: "" as keyof UpdateTargetMuscles,
    order: "asc",
  });
  ...
  const fetchFn = useCallback(async () => {
    return targetMuscleService.getAllTargetMuscles({
  ...
  const {
    ...
    loading,
    refetch,
  } = useTableFetch<PaginatedResponse<UpdateTargetMuscles>>(fetchFn, false);

  useEffect(() => {
    refetch();
  }, [fetchFn, refetch]);

  const [modalConfig, setModalConfig] = useState<TargetMuscleModalConfig>({
    isOpen: false,
    title: "",
    message: "",
    onConfirm: () => {},
  ...
  const actions = useTargetMuscleActions(refetch, setModalConfig);
  ...
  const handleSortChange = useCallback(
    (sort: SortConfig<keyof UpdateTargetMuscles>) => {
      setSortConfig(sort);
      setCurrentPage(1);
    },
    []
  ...
  const sortOptions = extractSortOptions(targetMuscleColumns);
    ...
    <>
      <div className="text-white">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-semibold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-400">Target Muscles Management</h1>
          <button
            onClick={() => navigate(ADMIN_UI_ROUTES.TARGET_MUSCLES_FORM)}
        ...
        <div className="mb-6 flex gap-4">
          ...
          />
          <SortDropdown<keyof UpdateTargetMuscles>
            options={sortOptions}
            value={sortConfig}
        ...
        {loading ? (
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <DataTable<UpdateTargetMuscles>
              columns={targetMuscleColumns}
              data={response?.data || []}
              actions={actions}
            />
...
export default AdminTargetMuscleList;
