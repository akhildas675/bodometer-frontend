...
import { BODY_REGION, BodyRegion } from "@/features/health/constants/fitness.constants";
import { TargetMuscle } from "@/features/admin/target-muscles/types/target-muscle.types";
...
const EMPTY_FORM: TargetMuscle = {
  title: "",
  description: "",
  image: null,
  bodyRegion: BODY_REGION.UPPER_BODY

}
const AdminTargetMuscleForm = () => {
  ...
  const [form, setForm] = useState<TargetMuscle>(EMPTY_FORM);
    ...
    if (isEdit && id) {
      const fetchTargetMuscle = async () => {
        try {
          ...
          toast.error("Failed to fetch target muscle");
        }
      };
      fetchTargetMuscle();
    }
  ...
  const handleSubmit = async () => {
    ...
    if (form.bodyRegion) formData.append("bodyRegion", form.bodyRegion);
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Describe this Muscle"
          ...
          <div>
            ...
            <input
              id="muscle-image-upload"
              type="file"
              accept="image/*"
              onChange={handleImageChange}
              className="hidden"
            />
            <label htmlFor="muscle-image-upload"
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-indigo-800/50 border-purple-600 rounded-lg px-4 py-2.5 text-white placeholder-purple-300 focus:outline-none focus:border-purple-400 transition disabled:opacity-50"
            >
              {Object.values(BODY_REGION).map((region) => (
...
export default AdminTargetMuscleForm;
