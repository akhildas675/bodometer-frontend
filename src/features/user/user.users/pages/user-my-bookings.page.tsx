...
const UserMyBookingsPage: React.FC = () => {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      // }
      setBookings([]);
      setPagination({ currentPage: 1, totalPages: 1, totalItems: 0, itemsPerPage: 10, hasNextPage: false, hasPreviousPage: false });
    } catch (error) {
  ...
  const handleCancelBooking = async (reason: string) => {
    ...
    try {
      ...
      // }
      toast.success("Booking functionality removed");
      setModalState({ type: null, bookingId: null });
    } catch (error) {
      toast.error(parseApiError(error).message);
    } finally {
