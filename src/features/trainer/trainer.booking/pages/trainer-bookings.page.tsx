...
const TrainerBookingsPage: React.FC = () => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await bookingService.confirmBooking(modalState.bookingId);
      if (res.success) {
  ...
  const handleReject = async (reason: string) => {
    ...
    try {
      const res = await bookingService.rejectBooking(modalState.bookingId, reason);
      if (res.success) {
  ...
  const handleComplete = async () => {
    if (!modalState.bookingId) return;
    try {
      const res = await bookingService.completeBooking(modalState.bookingId);
      if (res.success) {
