...
import InputBox from "@/ui.components/ui/input.box";
...
const TrainerSlotsPage: React.FC = () => {
  ...
  const fetchAvailabilities = async () => {
    ...
    try {
      ...
      const res = await bookingService.getAvailabilities(params);
  ...
  const handleCreateAvailability = async (e: React.FormEvent) => {
    ...
    setIsCreating(true);
    try {
      const res = await bookingService.createAvailability({
        startDate,
        endDate,
        timeWindows,
  ...
  const handleToggleAvailabilityStatus = async (availabilityId: string, currentStatus: boolean) => {
    try {
      const res = await bookingService.updateAvailabilityStatus(availabilityId, { isActive: !currentStatus });
      if (res.success) {
